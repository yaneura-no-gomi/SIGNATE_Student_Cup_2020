...
def trainer(fold, df):
    ...
    # ratio = ratio / ratio[2]
    # aug_num = ratio * len(train_df[train_df["labels"] == 2])
    # auged_0 = class_augmentation(train_df, 0, replace_num=3, aug_num=aug_num[0])
    # auged_1 = class_augmentation(train_df, 1, replace_num=3, aug_num=aug_num[1])
    # auged_3 = class_augmentation(train_df, 3, replace_num=3, aug_num=aug_num[3])

    # auged_df = pd.DataFrame()
    # for a in [auged_0, auged_1, train_df[train_df["labels"]==2], auged_3]:
