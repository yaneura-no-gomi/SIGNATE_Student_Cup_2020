...
def trainer(fold, df):
    ...
    train_weighted_bs = WeightedBatchSampler(train_dataset, batch_size=TRAIN_BATCH_SIZE, n_classes=NUM_CLASSES, sampling_weight=[0.23, 0.18, 0.20, 0.39])
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_sampler=train_weighted_bs
    )

    valid_weighted_bs = WeightedBatchSampler(valid_dataset, batch_size=VALID_BATCH_SIZE, n_classes=NUM_CLASSES, sampling_weight=[0.23, 0.18, 0.20, 0.39])
    valid_dataloader = torch.utils.data.DataLoader(
        valid_dataset, batch_sampler=valid_weighted_bs
    )
