...
def make_folded_df(csv_file, num_splits=5):
    ...
    # Under Sampling for training
    # もっとも少ないラベル(label = 1)のデータ数を取得
    label_num = df.groupby("labels").count().reset_index(drop=True)
    minimum_label = label_num.iloc[:, 0].min()

    # label = 0, 2, 3のデータからランダムにminimum_label個のデータを抽出しlabel=1とconcatしていく
    us_df = df[df["labels"] == 1]
    for l in [0, 2, 3]:
        idxes = df[df["labels"] == l].index
        random_indices = np.random.choice(idxes, minimum_label)
        us_df = pd.concat([us_df, df.loc[random_indices, :]])

    df = us_df.reset_index(drop=True)
