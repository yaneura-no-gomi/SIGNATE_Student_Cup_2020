...
from utils import check_submit_distribution, class_augmentation_retranslation
...
processed_data = os.path.join(
    os.environ["HOME"],
    "Workspace/learning/signate/SIGNATE_Student_Cup_2020/processed_data",
)
...
def trainer(fold, df):
    ...
    # ----- Train Augmentation ----- #
    nums = np.array([404, 320, 345, 674]) * 1.5
    nums = nums.astype('int32')
    print("* === Augmentaion & Under Sampling === *")
    auged_df = pd.DataFrame()
    for i in range(4):
        ori_num = len(train_df[train_df["labels"] == i])
        print(f"label={i}: {ori_num} -> {nums[i]}")

        if ori_num < nums[i]:
            # Augmentation
            tmp = class_augmentation_retranslation(
                df=train_df, target_job=i, aug_num=nums[i] - ori_num
            )
            auged_df = pd.concat([auged_df, tmp])

        else:
            # Under Sampling
            random_indices = np.random.choice(
                train_df[train_df["labels"] == i].index, nums[i]
            )
            tmp = train_df[train_df["labels"] == i].loc[random_indices, :]
            auged_df = pd.concat([auged_df, tmp])

    auged_df["id"] = list(range(len(auged_df)))
    train_df = auged_df.reset_index(drop=True)
    print(train_df)
    train_df.to_csv(os.path.join(processed_data, "retranslate_augmentation", "fold" + "_" + str(fold)+"_"+"train.csv"), index=False)
    ...
    valid_df = us_valid
    valid_df.to_csv(os.path.join(processed_data, "retranslate_augmentation", "fold" + "_" + str(fold)+"_"+"valid.csv"), index=False)
    ...
    criterion = nn.CrossEntropyLoss(
        weight=torch.tensor([1, 10, 1, 1], dtype=torch.float).to(DEVICE)
    )
